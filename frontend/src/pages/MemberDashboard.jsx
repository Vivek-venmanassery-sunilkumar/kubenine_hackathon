...
const MemberDashboard = () => {
  ...
  useEffect(() => {
    const handleEscKey = (event) => {
      if (event.key === 'Escape' && showSwapModal) {
            ...
            <div className="flex items-center justify-between p-6 border-b border-gray-200">
              <div>
                ...
                <p className="text-sm text-gray-600 mt-1">
                  Your slot: {formatDate(selectedSlot.start_datetime)} at {formatTime(selectedSlot.start_datetime)} - {formatTime(selectedSlot.end_datetime)}
                </p>
              </div>
              <button
                onClick={() => {
            ...
            <div className="p-6 overflow-y-auto max-h-[60vh]">
              <div className="space-y-4">
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  <h4 className="font-medium text-blue-900 mb-2">Your Current Slot</h4>
                  <div className="text-sm text-blue-800">
                    <div className="font-medium">{formatDate(selectedSlot.start_datetime)}</div>
                    <div>{formatTime(selectedSlot.start_datetime)} - {formatTime(selectedSlot.end_datetime)}</div>
                    <div>{selectedSlot.duration_hours} hours</div>
                  </div>
                </div>
                
                <div>
                  <h4 className="font-medium text-gray-900 mb-3">Available Slots to Swap With:</h4>
                  {availableSlots.length === 0 ? (
                    <div className="text-center py-8">
                      ...
                      <p className="text-gray-400 text-xs mt-1">All team members are already assigned or slots are in the past</p>
                    </div>
                  ) : (
                    <div className="space-y-3">
                      {availableSlots.map((slot) => (
                        <button
                          ...
                          className="w-full text-left p-4 bg-gray-50 hover:bg-gray-100 rounded-lg border border-gray-200 transition-colors hover:border-blue-300 hover:shadow-sm"
                        >
                          <div className="flex items-center justify-between">
                            <div>
                              <div className="font-medium text-gray-900">
                                {formatDate(slot.start_datetime)} - {formatTime(slot.start_datetime)} to {formatTime(slot.end_datetime)}
                              </div>
